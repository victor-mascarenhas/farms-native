...
export default function DashboardPage() {
  ...
  const vendasFiltradas = useMemo(() => {
    ...
    const diasAtras = new Date(
      hoje.getTime() - parseInt(periodo) * 24 * 60 * 60 * 1000
    );
    ...
    return sales.filter((sale: Sale) => {
      ...
      const produtoCorreto =
        filtroProduto === "todos" || sale.produto === filtroProduto;
  ...
  const lucroPorProduto = useMemo(() => {
    const map: Record<
      string,
      { vendas: number; lucro: number; quantidade: number }
    > = {};
    ...
    datasets: [
      {
        ...
        data: Object.keys(vendasPorDia)
          .sort()
          .map((data) => vendasPorDia[data]),
        borderColor: "#f59e0b",
  ...
  const estatisticas = useMemo(() => {
    const totalVendas = vendasFiltradas.reduce(
      (sum: number, sale: Sale) => sum + sale.valor,
      0
    );
    const totalLucro = Object.values(lucroPorProduto).reduce(
      (sum: number, p) => sum + p.lucro,
      0
    );
    const totalQuantidade = vendasFiltradas.reduce(
      (sum: number, sale: Sale) => sum + sale.quantidade,
      0
    );
